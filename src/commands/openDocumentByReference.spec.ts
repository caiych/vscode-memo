...
describe('openDocumentByReference command', () => {
  ...
  it('should create in a configured sub dir', async () => {
    await updateMemoConfigProperty('files.defaultPaths', [
      {
        rule: '.*\\.md$',
        comment: 'all notes',
        folder: '/Notes',
    ...
    const name = rndName(); // extension elided.
  ...
  it('should not create in a configured sub dir for long links', async () => {
    await updateMemoConfigProperty('files.defaultPaths', [
      {
        rule: '.*\\.md$',
        comment: 'all notes',
    ...
    const name = `dir/${rndName()}`; // extension elided.
  ...
  it('create in a root dir in case of no matching rule', async () => {
    await updateMemoConfigProperty('files.defaultPaths', [
      {
        rule: '.*\\.txt$',
        comment: 'all text',
    ...
    const name = `${rndName()}`; // extension elided, md is added by default
